...
const SignupPage = () => {
  ...
  } = useForm<SignUpInterface>({ mode: 'onChange' });
  ...
  // Signup logic
  const signup = async (formData: SignUpInterface) => {
    setLoading(true)
    try {
...
const deleteAccount = async (email:string) => {
  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/delete-account`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
          ...
          >
            <form onSubmit={handleSubmit(signup)} className="space-y-7">
              ...
              <motion.div
                ...
                className="text-center text-sm text-gray-400 mt-8 tracking-wide font-semibold uppercase">
